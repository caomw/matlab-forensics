...
for Quality=JPEG_Qualities
    ...
    Masks{FolderInd}=strrep( Masks{FolderInd},'/','\');        
        ...
        OutPath=['SyntheticData/' OutPath  '_' num2str(Quality) '_Res/'];
        mkdir(OutPath);
        ...
        for ii=1:min(length(List),upLimit)
            ...
            Ratio=[size(imSource ,1) size(imSource ,2)]./[2048 1024];
            [maxR,indR]=max(Ratio);
    
            if maxR>1
                im_res=imresize(imSource ,1/maxR, 'lanczos3');
            else
                im_res=imresize(imSource ,0.75, 'lanczos3');
            end
            
            imwrite(im_res,'tmpjpg.jpg','Quality',Quality);
            im=jpeg_read('tmpjpg.jpg');
            
            [Feature_Vector, OutputMap]=Extract_Features(im,false);
