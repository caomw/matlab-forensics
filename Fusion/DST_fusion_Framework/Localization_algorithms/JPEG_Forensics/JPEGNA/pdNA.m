...
for ts = 1:length(test_sizes)
    ...
    for t2 = 1:length(JPEGqualities)
        ...
        for thr_tmp = 0:th_inc:6
            acc_tmp = (sum(minH1_0(t2,1:nfiles) >= thr_tmp) + sum(sum(squeeze(minH1_1(1:end-1,t2,1:nfiles) < thr_tmp))) / (length(JPEGqualities)-1)) / (2*nfiles);
            if acc_max < acc_tmp
                acc_max = acc_tmp;
                thr1_max = thr_tmp;
            end
        ...
        thr2_max = 0;
        for thr_tmp = 0:th_inc:6
            acc_tmp = (sum(minH2_0(t2,1:nfiles) >= thr_tmp) + sum(minH2_1(t2,1:nfiles) < thr_tmp)) / (2*nfiles);
            if acc_max < acc_tmp
                acc_max = acc_tmp;
                thr2_max = thr_tmp;
            end
        ...
        for t1 = 1:length(JPEGqualities)
            if t1 == t2
                pd(t1,t2) = sum(minH1_0(t1,nfiles+1:2*nfiles) >= th1)/nfiles * sum(minH2_1(t1,1:nfiles) < th2)/nfiles;
    ...
    %             acc(t1,t2) = (sum(minH2_0(t2,1:nfiles) >= th2_eer) + sum(minH2_1(t2,1:nfiles) < th2_eer)) / (2*nfiles);
                acc_max = (sum(minH2_0(t2,1:nfiles) >= thr2_max) + sum(minH2_1(t2,1:nfiles) < thr2_max)) / (2*nfiles);
                acc(t1,t2) = acc_max;
            else
    ...
    %             acc(t1,t2) = (sum(minH1_0(t2,1:nfiles) >= th1_eer) + sum(squeeze(minH1_1(t1,t2,1:nfiles) < th1_eer))) / (2*nfiles);
                acc_max = (sum(minH1_0(t2,1:nfiles) >= thr1_max) + sum(squeeze(minH1_1(t1,t2,1:nfiles) < thr1_max))) / (2*nfiles);
                acc(t1,t2) = acc_max;
            end
        end 
    end
    ...
    accuracies_ZFA(ts,:) = (mean(pd(1:end-1,:))+1-pfa1)/2;
