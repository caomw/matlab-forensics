...
%read the first image name
imName = fgetl(imgNameFid);
if (strcmp(imName, 'saveDirPath'))
    %the next line defines the path to which the data should be saved
    dirName = fgetl(imgNameFid);
    %read the next line to get the image's name
    imName = fgetl(imgNameFid);
else
    dirName = '';
end
...
while (imName(1) ~= -1) && (0 == strcmp(imName, 'stopHere'))
   ...
   %Init the progress indicator
    ...
    saveName = [imName(slashLocation(end)+1:dotLocation(end)) 'fig'];
...
end %end of: while loop over images to analyze
