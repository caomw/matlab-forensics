function [F1Map, F2Map, CFAOut]=MarkSpliceDetect(GNoiseResidue,im)
    
    SmallCFAList={[2 1;3 2] [2 3;1 2] [3 2;2 1] [1 2;2 3]};
    CFAList=SmallCFAList;

    im=double(im(1:round(floor(end/2)*2),1:round(floor(end/2)*2),:));
    
    im=im(1:size(GNoiseResidue,1),1:size(GNoiseResidue,2),:);
