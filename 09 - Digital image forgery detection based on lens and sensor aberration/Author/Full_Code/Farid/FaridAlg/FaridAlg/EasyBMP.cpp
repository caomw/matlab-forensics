...
void PixelToPixelCopyNoBlue( BMP& From, int FromX, int FromY,  
                       BMP& To, int ToX, int ToY)
{
	*To(ToX,ToY) = *From(FromX,FromY);
	(To(ToX,ToY))->Blue = 0;
...
void RangedPixelToPixelCopyNoBlue (BMP& From, int FromL , int FromR, int FromB, int FromT, 
                             BMP& To, int ToX, int ToY )
{
 // make sure the conventions are followed
 if( FromB < FromT )
 ...
 { 
  for( i=FromL ; i <= FromR ; i++ )
  {
   PixelToPixelCopyNoBlue( From, i,j,  
                     To, ToX+(i-FromL), ToY+(j-FromT) );
  }
 }
...
bool RescaleBlueChannel( BMP& InputImage , char mode, double NewDimension )
{
 using namespace std;
 int CapMode = toupper( mode );
 ...
 InputImage.SetSize( NewWidth, NewHeight );
 InputImage.SetBitDepth( 24 );
 //restore the original image in the right place
if (NewDimension > 100) //enlargment
{
	int newWidthLocation = (NewWidth-OldWidth) / 2;
	int newHeightLocation = (NewHeight-OldHeight) / 2;
	RangedPixelToPixelCopyNoBlue(OldImage, 0 , OldWidth, 0, OldHeight,InputImage, 
								 newWidthLocation , newHeightLocation);
}
else
{
	//make sure the center of the cropped data from the original
	// image stays at the center of the resized (smaller) image
	int newWidthStart = (OldWidth-NewWidth) / 2;
	int newHeightStart = (OldHeight-NewHeight) / 2;
	//the excessive regions will be ignored by the call to the copy function
	RangedPixelToPixelCopyNoBlue(OldImage, newWidthStart , OldWidth, newHeightStart,  
								 OldHeight,InputImage,0 , 0);
}
 
 
 //InputImage.WriteToFile("C:\\Temp\\farid\\FaridAlg\\FaridAlg\\No BLUE.bmp");
 //OldImage.WriteToFile("C:\\Temp\\farid\\FaridAlg\\FaridAlg\\old image.bmp");
