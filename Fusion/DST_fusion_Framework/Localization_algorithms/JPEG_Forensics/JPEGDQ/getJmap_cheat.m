function [maskTampered, maskD] = getJmap_cheat(image,ncomp,c1,c2,q1table,alphatable, reltable)
...
%     title(['index = ' num2str(index) '; Q1 = ' num2str(Q1est) '; Q2 = ' num2str(Q2)])
    Q1est = q1table(floor((coe-1)/8)+1,mod(coe-1,8)+1);
    Q1 = Q1est;
...
%     pause
    biasest = bias;
    alpha = alphatable(floor((coe-1)/8)+1,mod(coe-1,8)+1);
...
%         ppt = mean(nhist) ./ (nhist + mean(nhist));
        ...
        prel = reltable(floor((coe-1)/8)+1,mod(coe-1,8)+1);
...
maskD = reshape(maskD,sizeCA(1)/8,sizeCA(2)/8);
