function [XT, XN, XTN] = DST_BBA_by_query(to_be_classified, training_array, training_model, forensic_algorithm)
%function [XT, XN, XTN] = DST_BBA_by_query(to_be_classified, training_array, training_model, forensic_algorithm)
%
...
%-> XTN: mass for the doubt
    ...
    if size(training_array,2)==3 && nargin>3
        sorted = sortrows([ training_array(:,1) training_array(:,2) get_tool_reliability(training_array(:,3),forensic_algorithm)],1);        
    else
        sorted = sortrows([ training_array(:,1) training_array(:,2)],1);        
    end
    
        nh = findNeighbors(to_be_classified,sorted,NN);
        %generate BBAs
...
function[neigh] = findNeighbors(target, array, N)
i=1;
try
    if target==0 || target<array(1,1)
        idx=1;
    else
    ...
    idxs = find(array(:,1)==array(i,1));
    if length(idxs)>1
        idx = idxs(floor(end/2));
    else
        idx = idxs;
    end
    ...
    secT = idx+N/2;
    %if  secB < 0
    %    neigh = array(idx - N/2- secB + 1 : idx + N/2,:);
    %elseif secT > size(array,1)
    %    neigh = array(idx - N/2 : idx + N/2 - abs(size(array,1)-secT) + 1,: );
    %else
    neigh = array(max(1,secB):min(size(array,1),secT),:);
    %end
catch
    a=0;
end
