function [OutputX, OutputY, dispImages, imin, Qualities, mins]=Ghost(imorig, checkDisplacements, smoothFactor)
    ...
    for ii=minQ:stepQ:maxQ
        ...
        Recompressed=Recompressor.RecompressImage(java.lang.Double(ii));
        pixelsData = reshape(typecast(Recompressed.getData.getDataStorage, 'uint8'), 4, width, height);
        pixelsData =double(pixelsData );
        
        tmpResave= cat(3, transpose(reshape(pixelsData(4, :, :), width, height)), transpose(reshape(pixelsData(3, :, :), width, height)), transpose(reshape(pixelsData(2, :, :), width, height)));
