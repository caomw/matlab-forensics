function[BBAF] = SimbYagerSum(BBA1, BBA2)
    ...
    %Assign conflict to universal set
    ...
    universal = [repmat('1',1,length(emptyset))];
    for k=1:size(BBAF.assignments,1)
        if(strcmp(BBAF.assignments{k,1},universal))
            BBAF.assignments{k,2} = BBAF.assignments{k,2} + BBAF.K;
        end
    end
