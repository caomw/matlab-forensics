...
%       Ido Yerushalmy
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Adapted for output by: Markos Zampoglou
% 

function [OutlierPrmsMap, OutlierPrmsMap_filtered, OutlierErrMap, OutlierErrMap_filtered, OutputStatistics]=findAberrations_compact(orgIm)

% OutputStatistics values:
%  1- average angular error
%  2- Number of analyzed PBA events
%  3- Average strength of PBA events
%  4- STD of PBA events' strength
%  5- Number of events per 10-degrees slice (relative to the
%     geometric center)
...
for outlierRemCtr = 1 : outliersItrNum
    ...
    options.MaxFunEvals='400*numberofvariables';
    ...
    outliersErr=[outliersPrms, [linesPrms(9:10, outliersIndices) ; err(outliersIndices)] ]; %#ok<AGROW>
...
OutputStatistics=[averageAngleErr, i, mean(err(1:i-1)),  std(err(1:i-1))];

bins = -180 : 10 : 180;
histRes = histc (rad2deg (angToGeoCntr), bins);
numOfBins = size(bins);
for binIndex = 1 : numOfBins(2)
    OutputStatistics=[OutputStatistics sum(histRes (binIndex,:))];
end
...
OutlierPrmsMap = zeros(imRows, imCols);
OutlierErrMap = zeros(imRows, imCols);
[r c] = size(outliersPrms);
for i=1 : c
    OutlierPrmsMap(round(outliersPrms(2,i)),round(outliersPrms(1,i))) ...
        = outliersPrms(3,i);
    OutlierErrMap(round(outliersErr(2,i)),round(outliersErr(1,i))) ...
        = outliersErr(3,i);
end
...
OutlierPrmsMap_filtered = imfilter(OutlierPrmsMap,mask);
OutlierPrmsMap_filtered = imfilter(OutlierPrmsMap_filtered,mask);

OutlierErrMap_filtered = imfilter(OutlierErrMap,mask);
OutlierErrMap_filtered = imfilter(OutlierErrMap_filtered,mask);

OutlierPrmsMap=single(OutlierPrmsMap);
OutlierPrmsMap_filtered=single(OutlierPrmsMap_filtered);

OutlierErrMap=single(OutlierErrMap);
OutlierErrMap_filtered =single(OutlierErrMap_filtered);